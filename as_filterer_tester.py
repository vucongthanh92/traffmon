...
def remove_from_range(ip_range, prio_ip_range):
    pass
...
def as_filterer():
    ...
    #################TEMPORAL###################
    as_ranges = json.loads(open("as_filterer_cache").read())

    for as_range in as_ranges.values():
        as_range["range"] = [ipaddress.IPv4Network(a) for a in as_range["range"]]
    #################TEMPORAL###################
                ...
                for prio_ip_range in prio_ranges:
                    if ip_range.overlaps(prio_ip_range):
                        ...
                        if ip_range <= prio_ip_range:
                            ...
                            prio["range"].sort()
                            #print( "This was unexpected D:. You should probably now remove this from ip_range instead. Check source!")
                            #raise
    ...
    for AS in as_ranges.values():
        ...
        is_too_big = prefix_count > (MAX_FILTER_LENGTH - FILTER_GUARD) / 22
        if is_too_big and not AS.get("exrange", []):
            ...
            for prefix in prefixes_list:
                ...
                if len(sfilter) + len(tail) > MAX_FILTER_LENGTH - FILTER_GUARD:
